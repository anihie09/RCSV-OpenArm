...
This code follows the joint position, velocity and torque from the arm over CAN FD
using the OpenArm CAN Library using the pattern at docs.openarm.dev/api-reference/can.


HARDWARE NOTE : I don't have access to OpenArm2.0 hardware so this includes LiveCANReader and MockCANReader.
Both have JointState dataclass and CANReader interface so sync, storage, dashboard is open from data from hardware
or simulations. 
In a real life sceneario where there is hardware, we just need to switch Switching MockCANReader for LiveCANReader in main.py.
...
@dataclass
class JointState:
    """records timestamp of joint feedback"""

    # monotonic seconds, host clock
    timestamp: float        
...
class LiveCANReader(CANReader):
    ...
    This is given for the next steps but there is no hardware.
    """
...
class MockCANReader(CANReader):
    ...
    """
    this is simulator for LiveCANReader.
    ...
    def _poll_loop(self) -> None:
        while self._running:
            ...
            #  torque: proportional to position + small damping term
...
if __name__ == "__main__":
    ...
    # smoke test: print joint states for around 2 seconds
